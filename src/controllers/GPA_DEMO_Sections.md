...
- Coustom LoadSQL : Criteria of filter also you have to mentioned key in sections of LOADSQL which 'autosql=N' then it will not access automatic LOadSQL it will acess coustom LoadSQL sections
- But always remember LoadSQL will compulsory mentioned in the Grid.   
### 5.4 Auto Join and Display Logic
  For fields such as party, prodid, shade, etc. that store PID:
...
-	Description: The WHERE clause defines the conditions used to filter the data when loading it into the grid. The filtering typically involves the use of parameters passed into the query (e.g., ?pid).
### -	Important: The condition in the WHERE clause can include dynamic values or placeholders such as ?sabid), which will be replaced during execution they take current sabid.
-	Example:
...
Sql = SELECT a.*, b.help_column FROM tablename a
       LEFT JOIN help_table b ON a.help_id = b.help_id
```
- In this example:
